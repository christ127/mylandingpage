...
export default function FormPage() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      const phone     = form.get("phone")?.toString() ?? ""; // optional
      const consent   = !!form.get("consent");               // checkbox
      ...
      await createSubmission({
        ...
        contentType: file.type || "image/jpeg",
        sizeBytes: file.size,
        // you can include phone as a custom field in your DTO/table if you added it
      });
    ...
    <main className="flex-1">
      <div className="form-wrap">
        <form onSubmit={onSubmit} className="form-card">
          ...
          {/* optional error message using existing style (no new styles added) */}
          {error ? <p className="form-hint" role="alert">Error: {error}</p> : (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="form-label">
              ...
              <input type="email" name="email" required className="form-input" placeholder="tucorreo@ejemplo.com" />
            </label>
          ...
          {/* Upload */}
          <div>
            <span className="form-label-text">Sube tu foto/recibo</span>
            <div onClick={() => fileRef.current?.click()} className="upload-box">
              {preview ? (
                <div className="flex items-center gap-4 justify-center">
                  <img src={preview} alt="preview" className="upload-preview" />
                  <span className="upload-hint">Haz clic para cambiar el archivo</span>
                </div>
              ) : (
                <div className="space-y-1">
                  <p className="upload-hint">Haz clic para seleccionar archivo (JPG/PNG)</p>
          ...
          <label className="flex items-start gap-3">
            {/* added name="consent" (not a style change) so we can read it from FormData */}
            <input type="checkbox" name="consent" required className="checkbox mt-[3px] h-4 w-4" />
            <span className="consent-text">
              Confirmo que acepto las{" "}
              <a href="#" className="link-green">reglas oficiales</a> y el consentimiento.
            </span>
          </label>
